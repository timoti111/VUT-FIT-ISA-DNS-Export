...
void ip_fragments::assembly_data(utils::memory_block& buffer, const uint16_t offset, const bool last_fragment)
{
    if (fragments_.find(offset) == fragments_.end())
    {
        ...
        if (whole_length_ == length_ && last_fragment_received_)
        {
            ...
            buffer = utils::memory_block(reassembled_data_.data(), reassembled_data_.size());
